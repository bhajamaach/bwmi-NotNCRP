...
export function GrievanceTimeline({ petition }: { petition: GrievancePetition }) {
  ...
  return (
    <ol className="relative">
      {grievanceStages.map((step, index) => {
        const update = petition.stageHistory.find((item) => item.stage === step.stage);
        const isLast = index === grievanceStages.length - 1;
        const state: "done" | "current" | "future" =
          index < currentIndex || (index === currentIndex && step.stage === "NOC_ISSUED")
            ? "done"
            : index === currentIndex
              ? "current"
              : "future";
